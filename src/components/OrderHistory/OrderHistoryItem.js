...
const OrderHistoryItem = (props) => {
        ...
        return(
            <div className="order-history-cart-item" key={index}>
                ...
                <div className="order-history-cart-total-wrapper">
                    ...
                    <div className="order-history-cart-price">{showPriceString+"Đ"}</div>
                </div>
            </div>
        )
    });
    ...
    let showTotalPriceString = new Intl.NumberFormat('de-DE').format(showTotalPrice);
    return(
        <div className="row">
            <div className="cell id" >{props.id+1}</div>
            <div className="cell orderCart">
                {loadOrderHistoryCart}
            </div>
